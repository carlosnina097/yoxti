          ...
          <div class="row">
            <div class="col-xs-12 col-lg-12">
              <form method="get" action="">
                ...
                $query = "SELECT e.nombre_empresa, e.ruc_empresa, e.url_empresa, e.imagen_empresa FROM empresa e limit $page1, 10";
                          $result = mysqli_query($dbcon, $query)
                  or die('Query failed: ' . mysqli_error($dbcon)); 
                $totalItem = 0;
                while ($tuple = mysqli_fetch_assoc($result)) {
                  print '<tr><td>'.$tuple['nombre_empresa'].'</td>
                  <td>'.$tuple['ruc_empresa'].'</td>
                  <td>'.$tuple['url_empresa'].'</td>
                  <td>'.$tuple['imagen_empresa'].'</td></tr>';
                  $totalItem += 1;
                }

                if ($totalItem < 1) {
                  print "<div class='jumbotron'><h4>No existen Empresas</h4></div>";
                ...
                for ($b=1;$b<=$p;$b++) {
                  ?><a href="empresa_list.php?page=<?php echo $b; ?>&cc="><?php echo $b." "; ?></a><?php
                }
                echo "<br>";
