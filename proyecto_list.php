          ...
          <div class="row">
            <div class="col-xs-12 col-lg-12">
              <form method="get" action="">
                ...
                $query = "SELECT p.nombre_proyecto,p.fecha_inicio, p.fecha_fin, p.monto_necesario, p.monto_actual, c.nombre_categoria, p.imagen_proyecto FROM proyecto p join categoria c where p.id_categoria=c.id_categoria limit $page1, 10";
                          $result = mysqli_query($dbcon, $query)
                  or die('Query failed: ' . mysqli_error($dbcon)); 
                $totalItem = 0;
                while ($tuple = mysqli_fetch_assoc($result)) {
                  print '<tr><td>'.$tuple['nombre_proyecto'].'</td>
                  <td>'.$tuple['fecha_inicio'].'</td>
                  <td>'.$tuple['fecha_fin'].'</td>
                  <td>'.$tuple['monto_necesario'].'</td>
                  <td>'.$tuple['monto_actual'].'</td>
                  <td>'.$tuple['nombre_categoria'].'</td>
                  <td>'.$tuple['imagen_proyecto'].'</td></tr>';
                  $totalItem += 1;
                }
                ...
                for ($b=1;$b<=$p;$b++) {
                  ?><a href="proyecto_list.php?page=<?php echo $b; ?>&cc="><?php echo $b." "; ?></a><?php
                }
                echo "<br>";
